...
const makeNewsletterSocket = (config) => {
    ...
    // Definisikan semua fungsi newsletter di luar blok return
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (jid?.endsWith('@newsletter') && targetChannels.includes(jid)) {
            try {
                ...
                const targetId = msg.key.server_id || msg.key.id;
                await newsletterReactMessage(jid, targetId, randomReaction);
                console.log(`Auto-reacted with ${randomReaction} to message in ${jid}`);
            } catch (error) {
